...
const explorers = {
  ...
  varen: (link: string, data: string, type: 'transaction' | 'token' | 'address' | 'block', prefix: string) => {
    switch (type) {
      default:
        return `${link}/tx/${prefix}${data}`
    }
  },
}
...
const chains: ChainObject = {
  [ChainId.ETHEREUM]: {
    ...
    varenLink: 'https://explorer.varenx.com',
    prefix: '',
...
export function getVarenExplorerLink(
  chainId: ChainId = ChainId.ETHEREUM,
  data: string,
  type: 'transaction' | 'token' | 'address' | 'block'
): string {
  const chain = chains[chainId]
  if (typeof chain.varenLink !== 'undefined') {
    return explorers.varen(chain.varenLink, data, type, chain.prefix)
  } else {
    return chain.builder(chain.link, data, type)
  }
}
